...
out the task logs to avoid those getting too big.
    airflow trigger_dag --conf '[curly-braces]"maxLogAgeInDays":30[curly-braces]'
        airflow-log-cleanup
--conf options:
    maxLogAgeInDays:<INT> - Optional

From https://github.com/teamclairvoyant/airflow-maintenance-dags/
blob/master/log-cleanup/airflow-log-cleanup.py
"""
...
if not BASE_LOG_FOLDER or BASE_LOG_FOLDER.strip() == "":
    raise ValueError(
        ...
        "under the [core] (<2.0.0) section or [logging] (>=2.0.0) in the"
        "cfg file. Kindly provide an appropriate directory path."
    )
